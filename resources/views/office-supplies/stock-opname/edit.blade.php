...
@section('title_page', 'Edit Stock Opname Session')
@section('breadcrumb_title', 'Edit Stock Opname Session')
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">Edit Session: {{ $session->session_code }}</h3>
                        </div>
                        <form action="{{ route('stock-opname.update', $session->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control @error('type') is-invalid @enderror" id="type"
                                                ...
                                                <option value="manual"
                                                    {{ old('type', $session->type) == 'manual' ? 'selected' : '' }}>Manual
                                                </option>
                                                <option value="scheduled"
                                                    {{ old('type', $session->type) == 'scheduled' ? 'selected' : '' }}>
                                                    Scheduled</option>
                                            </select>
                                            @error('type')
                                                <span class="invalid-feedback" role="alert">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group" id="schedule-type-group"
                                            style="display: {{ $session->type === 'scheduled' ? 'block' : 'none' }};">
                                            <label for="schedule_type">Schedule Type</label>
                                            <select class="form-control @error('schedule_type') is-invalid @enderror"
                                                id="schedule_type" name="schedule_type">
                                                <option value="">Select Schedule</option>
                                                <option value="monthly"
                                                    {{ old('schedule_type', $session->schedule_type) == 'monthly' ? 'selected' : '' }}>
                                                    Monthly</option>
                                                <option value="quarterly"
                                                    {{ old('schedule_type', $session->schedule_type) == 'quarterly' ? 'selected' : '' }}>
                                                    Quarterly</option>
                                                <option value="yearly"
                                                    {{ old('schedule_type', $session->schedule_type) == 'yearly' ? 'selected' : '' }}>
                                                    Yearly</option>
                                            </select>
                                            @error('schedule_type')
                                                <span class="invalid-feedback" role="alert">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                                        rows="3" placeholder="Enter session description">{{ old('description', $session->description) }}</textarea>
                                    @error('description')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" id="select-all-supplies"
                                            {{ count($selectedSupplies) === count($supplies) ? 'checked' : '' }}>
                                            ...
                                            <div class="col-md-4">
                                                <div class="form-check">
                                                    <input class="form-check-input supply-checkbox" type="checkbox"
                                                        ...
                                                        {{ in_array($supply->id, $selectedSupplies) ? 'checked' : '' }}>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes" rows="3"
                                        placeholder="Enter any additional notes">{{ old('notes', $session->notes) }}</textarea>
                                    @error('notes')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                            ...
                            <div class="card-footer">
                                ...
                                <a href="{{ route('stock-opname.show', $session->id) }}"
